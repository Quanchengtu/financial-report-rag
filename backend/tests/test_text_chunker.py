from app.services.text_chunker import chunk_text, split_into_paragraphs

# test 1 : 空白段落是否被移除
def test_split_into_paragraphs_removes_empty_parts():
    text = " First paragraph. \n\n   \n\nSecond paragraph.  "

    result = split_into_paragraphs(text)

    assert result == ["First paragraph.", "Second paragraph."]   # 驗證結果 

# test 2 : 短段落是否正確合併（短段落可以在不超過 chunk size 的情況下合併；超過上限時，則必須建立新的 chunk）
def test_chunk_text_merges_short_paragraphs_within_chunk_size():
    text = "Alpha.\n\nBeta.\n\nGamma."

    result = chunk_text(text, chunk_size=20, overlap=2)

    assert result == ["Alpha.\n\nBeta.", "Gamma."]   # 若加上gamma，chunk size = 21 已超過 20 不能合併

# test 3 : 錯誤參數是否被阻止
def test_chunk_text_validates_overlap_and_chunk_size():
    try:   # case 1: chunk size = 0
        chunk_text("abc", chunk_size=0, overlap=0)
        assert False, "Expected ValueError for chunk_size <= 0"  # 正常情況下上一行會出現錯誤而直接跳except 不會執行這行 若執行到這行表示上一行沒有出錯 這裡會強制讓測試執行失敗（預期出現錯誤value但實際上沒有）
    except ValueError as exc:
        assert "chunk_size" in str(exc)  # 確認是因為chunk size問題導致的error (exc是錯誤訊息物件)

    try:   # case 2: chunk_size == overlap
        chunk_text("abc", chunk_size=10, overlap=10)
        assert False, "Expected ValueError for overlap >= chunk_size"
    except ValueError as exc:
        assert "overlap" in str(exc)
