...
def score_chunk(question: str, chunk_text: str, section_name: str | None = None) -> dict:  # 若沒有傳入section_name預設是None 
    ...
    if section_name:
        if any(k in q for k in ["risk factor", "risk factors", "key risks", "major risks"]) and section_name == "item_1a_risk_factors":
            raw_score += 8
        elif any(k in q for k in ["market risk", "interest rate", "foreign exchange", "fx", "commodity"]) and section_name == "item_7a_market_risk":
            raw_score += 8
        elif any(k in q for k in ["business", "business model", "product", "customer"]) and section_name == "item_1_business":
            raw_score += 6
    ...
    return {
        "score": final_score,
        "matched_terms": sorted(set(matched_terms)) # set()去重 依字典序排序 短字串會靠前
    }
